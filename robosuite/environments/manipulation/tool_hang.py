...
from robosuite.utils.mjcf_utils import CustomMaterial, CustomMaterialFromPNG
from robosuite.utils.sim_utils import check_contact

from robosuite.environments.manipulation.single_arm_env import SingleArmEnv

from robosuite.models.arenas import TableArena
from robosuite.models.objects import StandWithMount, HookFrame, RatchetingWrenchObject, BoxObject
...
class ToolHang(SingleArmEnv):
    ...
    def _load_model(self):
        ...
        if self.patch_attack:
            ...
            patch_mat = CustomMaterialFromPNG(
                texture_png=self.patch_path,
                tex_name="patch",
                mat_name="patch_mat",
                tex_attrib=tex_attrib,
                mat_attrib=mat_attrib,
            )
            self.patch = BoxObject(name="patch",
                                   size=[self.patch_size, self.patch_size, 0.001],  # [0.015, 0.015, 0.015],
                                   material=patch_mat,)
            extra_objs.append(self.patch)
        ...
        self.model = ManipulationTask(
            ...
            mujoco_objects=[self.stand, self.frame, self.tool] + extra_objs,
        )
    ...
    def _get_placement_initializer(self):
        ...
        if self.patch_attack:
            ...
            # print("bin_x_half", bin_x_half, "bin_y_half", bin_y_half)
            self.placement_initializer.append_sampler(
                sampler=UniformRandomSampler(
                    name="PatchObjectSampler",
                    mujoco_objects=self.patch,
                    x_range=[-bin_x_half, 0],
                    y_range=[-bin_y_half, 0],
                    # x_range=[-0.115, -0.11],
                    # y_range=default_y_range,
                    rotation=[np.pi/2 + np.pi/4, np.pi/2-np.pi/4],
                    rotation_axis='z',
